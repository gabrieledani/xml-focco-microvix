...
nome_arquivo_xml = os.path.join(os.getcwd(),r'arquivos\LinxCadastraNcm.xml')
   ...
   <soapenv:Body>
      <tem:Importar>
			<tem:request>
				...
				<linx:Tabela>
					<linx2:Comando>LinxCadastraNcm</linx2:Comando>
					<linx2:Registros>'''
xml_produto.write(head)

nome_arquivo = os.path.join(os.getcwd(),r'arquivos\ncm.csv')
#print(nome_arquivo)
...
with  open(nome_arquivo, encoding="mbcs") as csv_file:
	...
	for row in csv_reader:
		...
		codigo = clean(codigo).zfill(8)

		descricao = clean(descricao)
		
		middle = '''	<linx:Registros>
							<linx:Colunas>
								<linx1:CommandParameter>
								...
								<linx1:CommandParameter>
									<linx1:Name>descricao</linx1:Name>
									<linx1:Value>'''+descricao[:60].upper()+'''</linx1:Value>
								</linx1:CommandParameter>
							</linx:Colunas>
						</linx:Registros>
                        '''
