   ...
   <soapenv:Body>
      <tem:Importar>
			<tem:request>
				...
				<linx:Tabela>
					<linx2:Comando>LinxCadastraColecoes</linx2:Comando>
					<linx2:Registros>'''
...
with  open(nome_arquivo, encoding="mbcs") as csv_file:
    ...
    for row in csv_reader:
        codigo, nome_colecao = row[0],row[1]
        
        codigo = clean(codigo)

        nome_colecao = clean(nome_colecao)
        ...
        middle = '''
						<linx:Registros>
							<linx:Colunas>
								...
								<linx1:CommandParameter>
									<linx1:Name>nome_colecao</linx1:Name>
									<linx1:Value>'''+nome_colecao.upper()+'''</linx1:Value>
								</linx1:CommandParameter>
							</linx:Colunas>
						</linx:Registros>'''
