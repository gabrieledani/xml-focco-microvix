...
nome_arquivo_xml = os.path.join(os.getcwd(),r'arquivos\LinxCadastraClassificacoes.xml')
   ...
   <soapenv:Body>
      <tem:Importar>
			<tem:request>
				...
				<linx:Tabela>
					<linx2:Comando>LinxCadastraClassificacoes</linx2:Comando>
					<linx2:Registros>'''
xml_produto.write(head)

nome_arquivo = os.path.join(os.getcwd(),r'arquivos\classificacao.csv')
#print(nome_arquivo)
...
with  open(nome_arquivo, encoding="mbcs") as csv_file:
	...
	for row in csv_reader:
		...
		codigo, nome_classificacao = row[0],row[1]
		
		codigo = clean(codigo)

		descricao = clean(nome_classificacao)
		...
		middle = '''
						<linx:Registros>
							<linx:Colunas>
								...
								<linx1:CommandParameter>
									<linx1:Name>nome_classificacao</linx1:Name>
									<linx1:Value>'''+nome_classificacao.upper()+'''</linx1:Value>
								</linx1:CommandParameter>
							</linx:Colunas>
						</linx:Registros>'''
